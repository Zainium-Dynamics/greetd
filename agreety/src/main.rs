...
fn get_distro_name() -> Result<String, Box<dyn std::error::Error>> {
    // Zainium: no real /etc -- try the real path first, then the plain
    // FHS one for portability on a real FHS host.
    let os_release = fs::read_to_string("/overlayer/syshub/etc/os-release")
...
fn get_issue() -> Result<String, Box<dyn std::error::Error>> {
    ...
    // Zainium: no real /etc -- try the real path first, then the plain
    // FHS one for portability on a real FHS host.
    Ok((fs::read_to_string("/overlayer/syshub/etc/issue")
        .or_else(|_| fs::read_to_string("/etc/issue"))?)
        .replace(
            "\\S",
