...
This is [Zainium Dynamics](https://zainiumdynamics.tech)' fork of
[Kenny Levinsen's greetd](https://git.sr.ht/~kennylevinsen/greetd),
adapted to run on [Zainium OS](https://zainiumdynamics.tech), which has
no real `/etc`, `/usr`, or `/bin` at the root — everything lives under
`/overlayer/syshub`. Changes from upstream:

- Every hardcoded FHS path (`/etc/pam.d`, `/etc/greetd`,
  `/etc/os-release`, `/etc/issue`, `/bin/sh`) now tries the real Zainium
  path first, falling back to the plain FHS path for portability on a
  real FHS host.
- Ported off `pam-sys` (C ABI / `dlopen("libpam.so.0")`) onto
  [`elevate-pam`](https://github.com/Zainium-Dynamics/elevate-privilege)'s
  own native Rust API directly — no runtime PAM library
  SONAME/dlopen/`$PATH` lookup at all anymore.
- Dropped `fakegreet` (unused on Zainium).

Licensed GPL-3.0-only, same as upstream — see [LICENSE](LICENSE) and
[NOTICE](NOTICE.md) for the full list of changes and their copyright.
