...
fn pam_service_exists(service: &str) -> bool {
    // Zainium: no real /etc or /usr at all -- the real PAM service files
    // live under /overlayer/syshub/etc/pam.d. Check that first, then fall
    // back to the plain FHS paths for portability on a real FHS host.
    Path::new(&format!("/overlayer/syshub/etc/pam.d/{}", service)).exists()
