...
use elevate_pam::constants::{PAM_CHANGE_EXPIRED_AUTHTOK, PAM_DELETE_CRED, PAM_ESTABLISH_CRED};
use serde::{Deserialize, Serialize};

/// pam-sys's old `PamFlag::NONE` was just 0 -- elevate-pam's flag
/// parameters are plain `i32`, no zero-value constant of their own.
const PAM_NO_FLAGS: i32 = 0;
...
fn worker(sock: &UnixDatagram) -> Result<(), Error> {
    ...
    if authenticate {
        pam.authenticate(PAM_NO_FLAGS)?;
    }
    match pam.acct_mgmt(PAM_NO_FLAGS) {
        Err(PamError::NewAuthTokenRequired(_)) => {
            pam.change_auth_token(PAM_CHANGE_EXPIRED_AUTHTOK)?;
        }
    ...
    match tty {
        ...
        TerminalMode::Terminal { path, vt, switch } => {
            ...
            pam.set_item_tty(&format!("tty{vt}"))?;
    ...
    let child = match unsafe { fork() }.map_err(|e| format!("unable to fork: {e}"))? {
        ...
        ForkResult::Child => {
            ...
            // Run
            // Zainium: no real /bin -- use the real path if it exists,
            // otherwise fall back to the plain FHS one for portability on
            // a real FHS host.
            let sh_path = if std::path::Path::new("/overlayer/syshub/bin/sh").exists() {
                "/overlayer/syshub/bin/sh"
            } else {
                "/bin/sh"
            };
            let cpath = CString::new(sh_path).unwrap();
    ...
    // inner-most child.
    pam.close_session(PAM_NO_FLAGS)?;
