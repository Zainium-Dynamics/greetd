...
pub fn read_config() -> Result<Config, Error> {
    ...
    let config_str = match matches.opt_str("config") {
        ...
        // Zainium: no real /etc -- try the real path first, then the
        // plain FHS ones for portability on a real FHS host.
        None => read_to_string("/overlayer/syshub/etc/greetd/greetd.conf")
            .or_else(|_| read_to_string("/overlayer/syshub/etc/greetd/config.toml"))
